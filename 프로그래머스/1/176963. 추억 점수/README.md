...
딕셔너리를 사용해서 각 사람별 그리움 점수를 저장한다. 딕셔너리를 [String:Int]로 설정하여 해당 name에 그리움 점수를 저장하는 방식이다.
각 사진마다 포함된 이름의 그리움 점수를 합산하고, 해당 이름이 딕셔너리에 없으면 기본값 0으로 처리한다.
최종 합산 점수를 결과 배열에 추가해서 결과를 구한다.
