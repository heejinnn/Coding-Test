...
1. 현재 자리수(digit)가 6 이상일 경우

- result += (10 - digit) → 올리는 횟수
- num += 10 → 올리기 때문에 다음 자리수에 1을 carry 해줌

2. 현재 자리수가 정확히 5일 경우
   
- 다음 자리수가 5 이상이면 → 올리는 게 더 유리
- 그렇지 않으면 → 그냥 내리는 쪽을 선택

3. 현재 자리수가 0~4일 경우
- 무조건 내리는 게 유리
- result += digit (그만큼 내림)

4. 처리 후에는 자리수를 한 칸 옮기기 위해:
