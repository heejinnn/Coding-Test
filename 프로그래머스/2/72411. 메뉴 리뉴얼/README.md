...
조합을 만들기 위해 단순히 두 개씩, 세 개씩 문자를 붙이는 방식으로 구현했음.
하지만 이 방식은 원하는 길이의 모든 조합을 만들지 못하고, 순서 의존적이거나 중복 제거가 정확히 되지 않음.

2. 최종 코드 플로우

1. 먼저 코스 후보 길이(courses)를 하나씩 순회함.

2. 각 길이에 대해:

- 조합을 담을 딕셔너리를 초기화함.
- 주문 목록을 순회하며, 각 주문에서 정렬된 문자 배열을 기준으로 DFS를 호출해 해당 길이의 조합을 모두 생성함.

3. DFS에서는 백트래킹 방식으로 조합을 만들어가며, 해당 조합이 완성되면 딕셔너리에 누적함.

4. 조합 생성이 끝나면, 해당 길이에서 가장 많이 등장한 조합의 등장 횟수(maxCount) 를 구하고, 이 값이 2 이상이면 가장 많이 등장한 조합들만 필터링해서 결과에 추가함.

5. 모든 course에 대해 반복한 뒤, 최종 결과를 정렬하여 반환함.
