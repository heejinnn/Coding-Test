...
이분 탐색을 활용해 숙련도의 최적 값을 찾는다. 
mid를 기준으로 총 퍼즐 소요 시간을 계산하고, 이 값이 제한 시간을 초과하면 숙련도를 높여야 하므로 start = mid + 1로 조정한다. 
반대로 제한 시간 이내라면 더 낮은 숙련도로도 가능할 수 있으므로 end = mid - 1로 줄이며 최적 값을 탐색한다. 이렇게 start가 end보다 커질 때까지 반복하여 가장 낮은 숙련도 중 조건을 만족하는 값을 찾는다.

- 총 퍼즐 소요 시간(totalTime) > limit인 경우 => start = mid + 1
- 총 퍼즐 소요 시간(totalTime) <= limit인 경우 => end = mid - 1
